...
from mazegen.maze_generator import Position, IS_42
...
class MazeGenerator:
    ...
    Parameters:
        - xvar: Main runtime object that holds references to rendering,
          windows, maze state, and algorithm instances.

    Attributes:
        - maze_generator.maze: Access the generated maze grid as a 2D list.

        - algorithm_object.shortest_path: Access the computed shortest
          path through the maze via the MazeAlgorithm instance.

    Notes:
        - The MazeGenerator itself sets up the maze structure, handles
          entry/exit positions, and special features (e.g., the 42 logo).

        - The actual maze layout and solution are generated by the assigned
          MazeAlgorithm.
    """

    def __init__(self, file: str, xvar: XVar) -> None:
        from mazegen.parsing.load_config import read_config_file
